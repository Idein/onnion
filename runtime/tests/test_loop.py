...
class SubGraph0:
    def __init__(self, opset_version, x):
    ...
    def run(self, iter_count, cond_in, y_in):
        ...
        [slice_out] = Slice(self.version).run(self.x, slice_start, slice_end)
    ...
    def to_onnx(self):
        ...
        x_const_node = onnx.helper.make_node(
            ...
            value=onnx.helper.make_tensor(
                ...
                dims=self.x.shape,
                vals=self.x.flatten().astype(float),
            ),
        )
...
def test_loop_00():
    ...
    attrs = {"body": SubGraph0(opset_version, x)}
    ...
    check(Loop, opset_version, attrs, inputs)
